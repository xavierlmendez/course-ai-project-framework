#!/usr/bin/env python3
"""Generate public and hidden tests for II-game (ABGame) from the reference solution,
...
case, so the professor can see which categories the twist is load-bearing in.
    ...
    python3 tests/make_tests.py            (run from part-II-game/)
"""
import os
import shutil
import subprocess
...
REF = os.path.join(PART, "reference", "solution", "ABGame.py")
CAN = os.path.join(PART, "canonical", "ABGame.py")
...
EXPECTED = os.path.join(EXAMPLE, "part-I-game/reference/solution/MiniMaxGame.py")

CASES = {
    "ab_game_pruning": [
        ("xxxWWWBxxBxWWBxxxxxxxBB", 2),
        ("xBxWxWBWBxxWxBxxWxBxxxx", 2),
        ("xWWxxxWxWxxBxxxBxBBWxBx", 2),
        ("BxBxWWxBxWxBxxWxxxBxWxx", 2),
        ("WxBWxWBBxxxxxxxBWWxxxBx", 2),
    ],
    "ab_game_mill": [
        ("WxWxBxBWxxxBWxBWxxBxxxx", 2),
        ("xxxBxBxxWxxxWxBWWBBxxxW", 2),
        ("BxxxxxBxBxWBxWxxWxBWxWx", 2),
        ("WxxBBBxBWxxxxxWxxWWxxBx", 2),
        ("xxBBxWxBxxxxxxWxWBBWxWx", 2),
        ("xxWxxxBxWBBxxxBxWBxWxWx", 2),
    ],
    "grad_depth3": [
        ("WxxWxxxxBxWxxxBBxWxxxBx", 3),
        ("xBxxxxxxWxBBBWxxxxWWxxx", 3),
        ("xBxxxWWBxBxxxxxxxWxxxBW", 3),
        ("xBxxxxWBxxxWWxBxxxxBxWx", 3),
    ],
}
