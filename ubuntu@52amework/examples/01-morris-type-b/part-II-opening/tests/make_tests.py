#!/usr/bin/env python3
"""Generate public and hidden tests for II-opening (ABOpening) from the reference solution,
...
case, so the professor can see which categories the twist is load-bearing in.
    ...
    python3 tests/make_tests.py            (run from part-II-opening/)
"""
import os
import shutil
import subprocess
...
REF = os.path.join(PART, "reference", "solution", "ABOpening.py")
CAN = os.path.join(PART, "canonical", "ABOpening.py")
...
EXPECTED = os.path.join(EXAMPLE, "part-I-opening/reference/solution/MiniMaxOpening.py")

CASES = {
    "ab_pruning": [
        ("xxxxxxxxxWxxxxxxBxxxxxx", 3),
        ("xxxxxxxxxxWxxxxxxxxxxxx", 3),
        ("xxxWWxxBxxxxxxxxxWBxxxB", 3),
        ("xWxxBxxBxxxxxxxxxBxxxWW", 3),
        ("xxBxxxBxxxxxxWWxWxBxxxx", 3),
    ],
    "ab_diagonal_mill": [
        ("xxWxxWxxxxxxxBBBxWxxxxx", 3),
        ("xxxxxWxWxxxxxWBBxxxBxxx", 3),
        ("WBxxBxWxxBxWxxxxxxxxxxx", 3),
        ("WxxxxxWxxWxxBBxxxxxxxxB", 3),
        ("xxxBxWxWxxBxxxxxxxxxxWB", 3),
        ("xxxxxBxxxxxWxBWxxxxxWxB", 3),
    ],
    "grad_depth4": [
