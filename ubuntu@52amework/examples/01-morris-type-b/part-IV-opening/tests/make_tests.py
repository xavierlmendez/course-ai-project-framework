#!/usr/bin/env python3
"""Generate public and hidden tests for IV-opening (MiniMaxOpeningImproved) from the reference solution,
...
case, so the professor can see which categories the twist is load-bearing in.
    ...
    python3 tests/make_tests.py            (run from part-IV-opening/)
"""
import os
import shutil
import subprocess
...
REF = os.path.join(PART, "reference", "solution", "MiniMaxOpeningImproved.py")
CAN = os.path.join(PART, "canonical", "MiniMaxOpeningImproved.py")
...
CASES = {
    "improved_valid": [
        ("xxxWxBxxWBxxxxBWWxBxxxx", 2),
        ("xxWxxxBxxxxxWxBWBxWxxxB", 2),
        ("xBBxxWBxBxxxxWxWxWxxxxx", 2),
        ("BxxxWxBBBWxWxxWxxxxxxxx", 2),
        ("xWBxxBxWWxxxxBxxWxxBxxx", 2),
    ],
    "improved_better": [
        ("WxWBxxxBxxxxWxxxxxxBxxx", 3),
        ("xxBxBxxBxxxxxxWWxWxxxxx", 3),
        ("xxxxxxxxWWBxWxxBxBxxxxx", 3),
        ("xBxxxxxxxBxWBxxWxxWxxxx", 3),
        ("WWxBxxxxBxxWBxxxxxxxxxx", 3),
        ("xxxxBWxxxxxxWWBxxxxxxBx", 3),
    ],
    "grad_depth3": [
        ("xWxxBxxxxxWxxxxWBBBxxWx", 3),
        ("xWxxxWxBxWxxxxxWBxBxBxx", 3),
        ("BWWxBxxxBxxxxWxxxxxxxWB", 3),
        ("xxxxxBWxxBWxBxBWxWxxxxx", 3),
    ],
}

POLICY = {
    "improved_valid": "valid",
    "improved_better": "estimate",
    "grad_depth3": "valid"
}
