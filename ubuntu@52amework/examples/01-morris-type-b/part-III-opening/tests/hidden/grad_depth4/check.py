#!/usr/bin/env python3
"""Equivalence policy `estimate` for the Morris variant, opening phase, Black to move.
...
Pass when:
  ...
  2. the board on line 1 is a legal result of ONE Black move on the input board under
  ...
  4. the MINIMAX estimate equals the reference estimate in expected.stdout.txt.
The positions-evaluated count is NOT compared here.
"""
import json
import re
import sys
...
PHASE = "opening"      # "opening" or "game" (midgame/endgame)
SIDE = "B"        # "W" or "B" -- the side to move at the root
MODE = "estimate"        # "estimate", "ab" or "valid"
...
def main():
    ...
    if a_board not in legal_results(board):
        print(f"board {a_board} is not a legal B result of {board}")
        return 1
