...
23. **The R620 is the harness verification machine**, container `harness-01`. Measurement on 2026-09-08 confirmed it: the temperature path works, and at 3.07 tok/s it cannot grade a cohort. Decision 9's dedicated grading machine is **still to be named** and is now the plan's largest open decision; the options and the throughput target are in `docs/review/evidence/grading-box.md`.
