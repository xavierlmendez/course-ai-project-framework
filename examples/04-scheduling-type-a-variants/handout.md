...
It writes `milestone-sched-a.json` in the directory you ran it from — the name comes from the `name` key of that `project.json`, so a record for one project can never overwrite another's. Submit `milestone-sched-a.json`. Auto-graded pass/fail.
