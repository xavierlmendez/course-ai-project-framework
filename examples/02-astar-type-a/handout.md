...
**http://localhost:8080/** from your laptop, **http://host.docker.internal:8080/** from inside the grading sandbox.

To practise at home, start that server yourself from the root of the course repository:

```
python3 tools/ledger_server.py --project examples/02-astar-type-a/project.json --port 8080
```

It prints the nonce it is serving and writes `examples/02-astar-type-a/ledger.tsv`. That is your own ledger; the graded one is the course server your instructor announces.
