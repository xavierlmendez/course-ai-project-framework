...
def best_game_move_black(board, depth):
    global positions_evaluated
    positions_evaluated = 0
    swapped = swap_colors(board)
    moves = generate_moves_midgame_endgame(swapped)
    best_board = None
    best_val = -float('inf')
    for m in moves:
        val = minimax_game(m, depth - 1, False)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 MiniMaxGameBlack.py <input> <output> <depth>")
        sys.exit(1)
    ...
    best_board, estimate = best_game_move_black(board, depth)
