...
# ---------------------------------------------------------------------------
# Alpha-beta search (fail-soft)
# ---------------------------------------------------------------------------

positions_evaluated = 0


def alphabeta_opening(board, depth, alpha, beta, is_max_player):
    ...
    if is_max_player:
        ...
        best = -float('inf')
        for m in moves:
            val = alphabeta_opening(m, depth - 1, alpha, beta, False)
            if val > best:
                best = val
            if best > alpha:
                alpha = best
        ...
        best = float('inf')
        for m in moves:
            val = alphabeta_opening(m, depth - 1, alpha, beta, True)
            if val < best:
                best = val
            if best < beta:
                beta = best
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 ABOpening.py <input> <output> <depth>")
        sys.exit(1)
