...
def count_mills_and_threats(board, color):
    mills = 0
    threats = 0
    for mill in MILLS:
        pieces = sum(1 for pos in mill if board[pos] == color)
        empties = sum(1 for pos in mill if board[pos] == 'x')
        if pieces == 3:
            mills += 1
        elif pieces == 2 and empties == 1:
            threats += 1
    return mills, threats


def static_estimation_opening_improved(board):
    num_white = board.count('W')
    num_black = board.count('B')
    w_mills, w_threats = count_mills_and_threats(board, 'W')
    b_mills, b_threats = count_mills_and_threats(board, 'B')
    return ((num_white - num_black)
            + 10 * (w_mills - b_mills)
            + 5 * (w_threats - b_threats))
...
def best_opening_move(board, depth):
    global positions_evaluated
    positions_evaluated = 0
    moves = generate_moves_opening(board)
    best_board = None
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 MiniMaxOpeningImproved.py <input> <output> <depth>")
        sys.exit(1)
    ...
    best_board, estimate = best_opening_move(board, depth)
