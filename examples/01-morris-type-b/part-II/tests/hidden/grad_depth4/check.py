#!/usr/bin/env python3
"""Equivalence policy `ab` for ABOpening.

    python3 check.py <in.txt> <expected.stdout.txt> <actual.json>

expected.stdout.txt is produced by the MINIMAX reference (MiniMaxOpening) on the
same board and depth. Pass when the actual output is well formed, the board is a
legal White result, the estimate equals the minimax estimate, the output file
matches, AND the positions-evaluated count is strictly lower than minimax's.
"""
import json
import re
import sys
