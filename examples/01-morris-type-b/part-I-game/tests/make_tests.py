#!/usr/bin/env python3
"""Generate public and hidden tests for I-game (MiniMaxGame) from the reference solution,
...
case, so the professor can see which categories the twist is load-bearing in.
    ...
    python3 tests/make_tests.py            (run from part-I-game/)
"""
import os
import shutil
import subprocess
...
REF = os.path.join(PART, "reference", "solution", "MiniMaxGame.py")
CAN = os.path.join(PART, "canonical", "MiniMaxGame.py")
...
CASES = {
    "game_move": [
        ("xxWBxxWxBWBxBWWxxxBxxxx", 2),
        ("BBxxWWxBBWxxxBxWWxxxxxx", 2),
        ("BxxxxxxWWxWBxBWxxBxxWBx", 2),
        ("xxxWxWBxxBxxxBBxBWxWxWx", 2),
        ("BBxWxWxBxBxWBxxxxxxxWWx", 2),
    ],
    "game_hopping": [
        ("xBxxxxxxBxWxxBxxBWBWxxx", 2),
        ("xBxxBBxxBxxxxxWBxWxxWxx", 2),
        ("BBxWxxBxWxxBxxBxxWxxxxx", 2),
        ("xxxxxWxxBBxWxBxBWxxxxxB", 2),
    ],
    "game_mill": [
        ("xxBxBWxxBxWxxxxWxWxBWBx", 2),
        ("xBxxWxxBBWxxxxxBWBWxxxW", 2),
        ("xxBxxBWBxxxxWxBxWBxxWxW", 2),
        ("WBxxWxBxxxxBxxWxxxWBWBx", 2),
        ("BxWxBxxWxxxWWBWxxxxxBxB", 2),
        ("WWBxWxWBxxBxxWBBxxxxxxx", 2),
    ],
    "grad_depth3": [
        ("xBWxxxxxxxBxBxWxWBxWxxx", 3),
        ("WxxBWxBWxxxxxWxxxxBxxBx", 3),
        ("WxxBxxxxxxBxWBxxxxWWxBx", 3),
        ("xxWBxxxWxxxWBBxxxxWxxBx", 3),
    ],
}

POLICY = {
    "game_move": "estimate",
    "game_hopping": "estimate",
    "game_mill": "estimate",
