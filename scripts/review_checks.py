...
def course_dir_of(pdir):
    """The course directory a part belongs to: the parent, when it holds parts.json and the
    course-level submissions/ tree students submit into. None for a single-part project."""
    parent = os.path.dirname(os.path.normpath(pdir))
    if os.path.exists(os.path.join(parent, "parts.json")) and \
       os.path.isdir(os.path.join(parent, "submissions")):
        return parent
    return None


def check_course_submissions():
    """The course-level submission tree of every multi-part example: does it match the layout
    the handout asks for, and does the pre-scan read it cleanly? These are the two commands a
    TA runs on Day 0, before anything has been copied into the per-program directories."""
    h("Course-level submissions (the layout students submit)")
    table_header()
    courses = sorted({os.path.dirname(p) for p in
                      glob.glob(os.path.join(ROOT, "examples", "*", "parts.json"))
                      if os.path.isdir(os.path.join(os.path.dirname(p), "submissions"))})
    if not courses:
        row("SKIP", "multi-part examples with a course-level submissions/", "none")
        return
    for course in courses:
        label = os.path.relpath(course, ROOT)
        code, out, err = sh([sys.executable, FAN_OUT, "--project", course,
                             "--submissions", os.path.join(course, "submissions"), "--check"])
        detail = (out.strip().splitlines() or [""])[-1]
        row("PASS" if code == 0 else "FAIL", f"{label}: fan_out.py --check (student layout)",
            detail[:200] if code == 0 else (out + err).strip().replace("\n", " · ")[-300:])
        code, out, err = sh([sys.executable, PRESCAN, "--course", course])
        if code != 0:
            row("FAIL", f"{label}: prescan --course", (err or out).strip()[-200:])
            continue
        for line in out.splitlines():
            status, sid = (line.split("\t") + ["", ""])[:2]
            row("PASS" if status == "OK" else "WARN",
                f"{label}/submissions/{sid}: prescan --course", line.split("\t", 2)[-1][:200])
...
def check_examples():
    ...
    for label, path in find_projects():
        ...
        # sample submissions: word cap + prescan. A part of a multi-part project has no
        # submissions/ of its own that anyone maintains — fan_out.py generates it from the
        # course-level tree — so it is checked once per course in check_course_submissions().
        subs = os.path.join(pdir, "submissions")
        if course_dir_of(pdir):
            subs = None
        if subs and os.path.isdir(subs):
